...
export function WorkspaceCollectionItemCard({
    ...
    const content = (
        <div
            className={cn(
                "group relative flex h-full w-full max-w-full flex-col gap-2 overflow-hidden rounded-lg border bg-card/60 p-2.5 text-left transition-colors sm:gap-3 sm:rounded-xl sm:p-3.5",
                selected
        ...
        >
            ...
            {item.metadata?.length ? (
                <div className="grid w-full max-w-full grid-cols-2 gap-1 overflow-hidden text-xs text-muted-foreground">
                    {item.metadata.slice(0, 4).map((entry) => (
                        <div key={`${item.id}-${entry.label}`} className="min-w-0 max-w-full space-y-0.5 overflow-hidden rounded-md border border-border/60 bg-background/70 px-1.5 py-1 sm:rounded-lg sm:px-2 sm:py-1.5">
