...
import { generateWireListPdfExports } from "@/lib/project-exports/wire-list-pdf-exports";
...
async function handleDownloadAllWireLists(
  request: NextRequest,
  ...
  try {
    const exportResult = await generateWireListPdfExports(projectId, request.nextUrl.origin, { grouping });
    if (!exportResult.sheetExports.length) {
      return NextResponse.json(
        { error: "No wire-list exports were generated for this project." },
        { status: 404 },
      );
    }
    ...
    const fileName = `${safeProjectName || "project"}-wirelists${groupingSuffix}.zip`;
    ...
    console.error("[download-all] Export failed:", error);
    const message = error instanceof Error ? error.message : "Failed to generate wire-list zip export";
...
export async function GET(
  ...
  return handleDownloadAllWireLists(request, params);
}
