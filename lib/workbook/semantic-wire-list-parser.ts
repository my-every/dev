  ...
  for (const row of introRows) {
    ...
    switch (key) {
      ...
      case "controls me":
        ...
        break;
      case "before workbook":
        metadata.beforeWorkbook = value;
        break;
      case "after workbook":
        metadata.afterWorkbook = value;
        break;
      case "phone":
...
export function buildSemanticWireListRows(
  ...
  const normalizedHeaders = (headerRow ?? []).map(cell => normalizeHeaderLookupToken(normalizeHeaderCell(cell)));
  const stampColumnIndex = normalizedHeaders.findIndex(header => header === "stamp");
  const previousValuesColumnIndex = normalizedHeaders.findIndex(header => header === "previous field values");
  const updatedColumnIndex = normalizedHeaders.findIndex(header => header === "updated?");
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    for (const [colIndexStr, fieldName] of Object.entries(columnMap)) {
      ...
      if (colIndex < rawRow.length) {
        ...
        (semanticRow as unknown as Record<string, string | number>)[fieldName] = strValue;
      }
    }

    if (stampColumnIndex >= 0 && stampColumnIndex < rawRow.length) {
      const stampValue = normalizeHeaderCell(rawRow[stampColumnIndex]);
      semanticRow.greenChangeStamp = stampValue;
      semanticRow.greenChangeType = normalizeGreenChangeType(stampValue);
    }

    if (previousValuesColumnIndex >= 0 && previousValuesColumnIndex < rawRow.length) {
      semanticRow.greenChangePreviousValues = normalizeHeaderCell(rawRow[previousValuesColumnIndex]);
    }

    if (updatedColumnIndex >= 0 && updatedColumnIndex < rawRow.length) {
      semanticRow.greenChangeUpdated = normalizeHeaderCell(rawRow[updatedColumnIndex]);
    }
...
function normalizeHeaderLookupToken(value: string): string {
  return value.toLowerCase().replace(/["'`]/g, "").replace(/\s+/g, " ").trim();
}

function normalizeGreenChangeType(stampValue: string): SemanticWireListRow["greenChangeType"] {
  const normalized = stampValue.toLowerCase().trim();
  if (!normalized) {
    return undefined;
  }

  if (normalized === "added" || normalized === "add") {
    return "added";
  }
  ...
  if (normalized === "chg" || normalized === "changed" || normalized === "modified" || normalized === "updated") {
    return "changed";
  }
...
export function parseSemanticWireList(
  ...
  const semanticRows = buildSemanticWireListRows(dataRows, columnMap, headerRowIndex, headerRow);
