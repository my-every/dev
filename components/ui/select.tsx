...
import type { IconComponent } from "@/lib/theme/icon-context";
...
const SelectTrigger = forwardRef<HTMLButtonElement, SelectTriggerProps>(
  (
    ...
    const valueChild = Children.toArray(children).find((child) =>
      isValidElement(child) && (child.type as { displayName?: string })?.displayName === "SelectValue"
    ) as React.ReactElement<SelectValueProps> | undefined;
    const valuePlaceholder = valueChild?.props?.placeholder;
    const valueFallback = valueChild?.props?.children;
    const label = value ? labelMap.current.get(value) ?? value : undefined;
    const resolvedLabel = label ?? valueFallback;
      ...
      >
        <span className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <span className="min-w-0 flex-1 text-left truncate">
            {resolvedLabel ?? (
              <span className="text-muted-foreground">{valuePlaceholder ?? placeholder}</span>
            )}
          </span>
        </span>

        <ChevronDownIcon className="size-4 shrink-0 text-muted-foreground transition-colors duration-80 group-hover:text-foreground" />
      </button>
      {error && (
...
const SelectContent = forwardRef<HTMLDivElement, SelectContentProps>(
  ({ className, children }, ref) => {
    ...
    // Close when any scrollable ancestor of the trigger scrolls so the dropdown
    // does not remain open at a stale fixed position. window alone is not enough
    // because dialogs use overflow-y-auto panels that scroll without moving the window.
    useEffect(() => {
      if (!open || !triggerRef.current) return;
      const onScroll = () => setOpen(false);
      const scrollables: EventTarget[] = [window];
      ...
      scrollables.forEach((s) => s.addEventListener("scroll", onScroll, { passive: true }));
      return () => scrollables.forEach((s) => s.removeEventListener("scroll", onScroll));
    }, [open, setOpen, triggerRef]);
...
// ---------------------------------------------------------------------------
// Radix-style compatibility components
// ---------------------------------------------------------------------------
...
const SelectScrollUpButton = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
...
const SelectScrollDownButton = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
