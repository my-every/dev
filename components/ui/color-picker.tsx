...
// Compatibility layer for the component
const SlotPrimitive = { Slot };
const DirectionPrimitive = { useDirection };
...
function ColorPickerSwatch(props: DivProps & { value?: string }) {
  const { asChild, className, value, ...swatchProps } = props;
  ...
  // If a value prop is provided, use it as a preset color; otherwise show current color
  const displayColor = value ? hexToRgb(value) : currentColor;
  ...
  const backgroundStyle = React.useMemo(() => {
    ...
    const colorString = `rgba(${displayColor.r}, ${displayColor.g}, ${displayColor.b}, ${displayColor.a})`;
  ...
  const ariaLabel = !displayColor
    ? "No color selected"
    : value 
      ? `Select color: ${value}` 
      : `Current color: ${colorToString(displayColor, format)}`;

  // Handle click to set the preset color
  const handleClick = React.useCallback(() => {
    if (value && !context.disabled) {
      const newColor = hexToRgb(value);
      const newHsv = rgbToHsv(newColor);
      store.setColor(newColor);
      store.setHsv(newHsv);
    }
  }, [value, context.disabled, store]);

  // If value is provided, render as a clickable button
  if (value) {
    return (
      <button
        type="button"
        ...
        disabled={context.disabled}
        className={cn(
          "box-border size-8 rounded-sm border shadow-sm cursor-pointer hover:scale-110 hover:ring-2 hover:ring-primary/50 transition-transform focus:outline-none focus:ring-2 focus:ring-primary",
