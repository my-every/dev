...
export const FileCard = ({ formatFile, size = "default", className }: FileCardProps) => {
    ...
    const sizeClass = size === "compact" ? "h-14 w-11 p-1.5" : "w-14 h-18 p-2";
    const badgeClass = size === "compact" ? "-right-1.5 bottom-1 px-1 py-0.5 text-[7px]" : "-right-2 bottom-1.5 px-1.5 py-0.5 text-[8px]";

    return (
        <div aria-hidden className="relative size-fit">
            <div
                className={cn(
                    "absolute z-2 rounded font-medium uppercase",
                    badgeClass,
                    colorBannerClass
                )}
            >
            ...
            <div
                className={cn(
                    "dark:bg-secondary ring-border relative z-1 space-y-3 rounded-md bg-white ring-1",
                    sizeClass,
                    className
                )}
            >
