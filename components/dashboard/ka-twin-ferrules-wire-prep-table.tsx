...
interface KaTwinFerrulesWirePrepTableProps {
  title: string;
  instruction: string;
...
function getGroupKey(row: WirePreparationRow): string {
  return (row.fromDeviceId || "").trim().toUpperCase();
}

function buildGroups(rows: WirePreparationRow[]): KaTwinGroup[] {
  if (rows.length === 0) {
    return [];
  }

  const groups: KaTwinGroup[] = [];
  ...
  for (let index = 1; index < rows.length; index += 1) {
    ...
    groups.push({
      id: `${currentRows[0].key}-${groups.length}`,
      fromDeviceId: currentRows[0].fromDeviceId || "-",
      rows: currentRows,
    });
    currentRows = [row];
    currentKey = rowKey;
...
export function KaTwinFerrulesWirePrepTable({
  title,
  instruction,
  rows,
  maxRows = 400,
  prepCount,
  imageSrc,
}: KaTwinFerrulesWirePrepTableProps) {
    ...
    <Card className="w-full max-w-lg min-w-0 break-inside-avoid-column max-h-max gap-2 rounded-none py-2 shadow-none print:break-inside-avoid-page">
      <CardHeader className="gap-1 px-3">
        <div className="flex items-start justify-between gap-2">
          ...
          {imageSrc ? <img src={imageSrc} alt="KA twin ferrules" className="h-12.5 w-12.5 object-cover" /> : null}
        </div>
      </CardHeader>
