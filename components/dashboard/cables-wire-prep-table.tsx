...
interface CablesWirePrepTableProps {
  title: string;
  instruction: string;
...
interface CableWirePrepGroup {
  id: string;
  rows: WirePreparationRow[];
}

interface DisplayCableRow {
...
function buildDisplayRows(rows: WirePreparationRow[]): DisplayCableRow[] {
...
function buildCableGroups(rows: WirePreparationRow[]): CableWirePrepGroup[] {
  if (rows.length === 0) {
    return [];
  }

  const groups: CableWirePrepGroup[] = [];
...
export function CablesWirePrepTable({
  title,
  instruction,
  rows,
  maxRows = 400,
  prepCount,
  imageSrc = "/wire-prep/cables.png",
}: CablesWirePrepTableProps) {
  const filteredRows = rows.filter((row) => !isEthernetWireId(row.wireId));
  const previewRows = maxRows > 0 ? filteredRows.slice(0, maxRows) : filteredRows;
  const groups = buildCableGroups(previewRows);
    ...
    <Card className="w-full max-w-lg min-w-0 break-inside-avoid-column max-h-max gap-2 rounded-none py-2 shadow-none print:break-inside-avoid-page">
      <CardHeader className="gap-1 px-3">
        <div className="flex items-start justify-between gap-2">
          ...
          {imageSrc ? <img src={imageSrc} alt="Cable" className="h-12.5 w-12.5 object-cover" /> : null}
        </div>
      </CardHeader>
