...
interface GroundsWirePrepTableProps {
  title: string;
  instruction: string;
...
export function GroundsWirePrepTable({
  title,
  instruction,
  rows,
  maxRows = 400,
    ...
    <Card className="w-full max-w-lg min-w-0 break-inside-avoid-column max-h-max gap-2 rounded-none py-2 shadow-none print:break-inside-avoid-page">
      <CardHeader className="gap-1 px-3">
        <div className="flex items-start justify-between gap-2">
          ...
          {imageSrc ? <img src={imageSrc} alt="Ground" className="h-12.5 w-12.5 object-cover" /> : null}
        </div>
      </CardHeader>
          ...
          <table className="w-full table-fixed border-collapse text-[10px] leading-tight">
            ...
            <tbody>
              {previewRows.map((row, index) => (
                <tr key={row.key} className="border-b border-b-border last:border-b-0">
