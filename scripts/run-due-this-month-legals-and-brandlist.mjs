...
async function loadReadyProjects() {
  const raw = await fs.readFile(REPORT_PATH, "utf8");
  const parsed = JSON.parse(raw);
  const projects = Array.isArray(parsed?.details?.dueThisMonthFullyReady)
    ? parsed.details.dueThisMonthFullyReady
    : [];
  ...
  return projects.map((entry) => ({
    ...
    dueDates: Array.isArray(entry.dueDates) ? entry.dueDates : [],
  })).filter((entry) => entry.pdNumber);
}

async function main() {
  if (!SKIP_REPORT) {
    console.log("Refreshing due-this-month availability report...");
    runCommand("node", [REPORT_SCRIPT, "--json", REPORT_PATH]);
  }

  const projects = await loadReadyProjects();
  console.log(`Found ${projects.length} due-this-month fully-ready projects.`);
  ...
  for (const project of projects) {
    const brandListDir = path.join(BRAND_LIST_ROOT, project.pdNumber);
    console.log(`\n=== ${project.pdNumber} | ${project.projectName || "(no project name)"} | ${project.dueDates.join(", ") || "(no due date)"} ===`);
    ...
    if (DRY_RUN) {
      ...
      results.push({ pdNumber: project.pdNumber, status: "dry-run" });
      continue;
    }
  ...
  console.log(`\nBatch summary: ${completed}/${results.length} completed, ${failed.length} failed.`);
